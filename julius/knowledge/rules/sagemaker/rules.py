...
def detect(account: Account, config: Config, scan_id: str) -> list[Opportunity]:
    ...
    for app in account.sagemaker_apps:
        ...
        if (
            ...
            and app.idle_hours_per_day >= th.sm_idle_hours_min
            and idle_off
        ):
...
def _idle_app(account: Account, app: SageMakerApp, config: Config, scan_id: str) -> Opportunity:
    ...
    return build(
        Finding(
            ...
            rule_id="SM-APP-IDLE",
            rule_version="1.0.0",
            title=f"{app.app_type} ocioso sem idle shutdown",
            why=(
        ...
        Recommendation(
            difficulty=1,
            action="Habilitar idle shutdown (ou reduzir o período)",
        ...
        Evidence(
            ...
            sources=["SageMaker ListApps", "CloudWatch"],
            observed_runs=app.active_days_per_month,
            coverage_days=app.coverage_days,
            has_optional_metrics=app.idle_hours_per_day > 0,
...
def _unused_endpoint(
    account: Account, ep: SageMakerEndpoint, config: Config, scan_id: str
) -> Opportunity:
    ...
    return build(
        Finding(
            ...
            asset_name=ep.name,
            rule_id="SM-ENDPOINT-UNUSED",
            rule_version="1.0.0",
            title="Endpoint em tempo real praticamente sem uso",
            why=f"Endpoint com {ep.instance_count}× {ep.instance_type} roda 24/7 mas teve {ep.invocations_per_month} invocações/mês.",
        ),
        Recommendation(
            difficulty=2,
            action="Descomissionar ou migrar para Serverless/Async Inference",
            ...
            docs=[_DOC_SERVERLESS, _DOC_ENDPOINT],
            risk=0.7,
        ),
        Evidence(
            items=[
            ...
            sources=["SageMaker DescribeEndpoint", "CloudWatch Invocations"],
            observed_runs=max(1, ep.coverage_days // 7),
            coverage_days=ep.coverage_days,
            has_optional_metrics=ep.coverage_days >= config.thresholds.min_coverage_days,
            owner_tag=ep.owner_tag,
        ),
        est,
        RuleContext(
            account=account.account_id,
