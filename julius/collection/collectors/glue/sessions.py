...
def collect_sessions(
    ...
    *,
    ...
    listagem = safe_pages(glue_client, "list_sessions", "Sessions")
    if gaps is not None and not listagem.complete:
        gaps.append(f"list_sessions: {listagem.error_category or 'incompleto'}")
    for s in listagem.items:
        max_capacity = (
            float(s["MaxCapacity"]) if s.get("MaxCapacity") is not None else None
        )
        workers = (
            int(s["NumberOfWorkers"]) if s.get("NumberOfWorkers") is not None else None
        )
