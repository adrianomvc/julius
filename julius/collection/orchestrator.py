...
from julius.collection.collectors.athena import monthly as athena_monthly
from julius.collection.collectors.glue import (
    cost as glue_cost,
)
from julius.collection.collectors.glue import (
    crawlers as glue_crawlers,
)
from julius.collection.collectors.glue import (
    databrew,
    jobs,
...
def collect_account(
    ...
    *,
    ...
    account.glue_jobs = health.capture(
        "Glue Jobs",
        lambda: jobs.collect_jobs(glue, window=window),
        _nothing(),
        required=True,
        count=len,
        data_through=_latest_data_through,
    ...
    account.tables = health.capture(
        "Glue Catalog",
        lambda: jobs.collect_tables(glue),
        _nothing(),
        count=len,
    ...
    account.glue_crawlers = health.capture(
        "Glue Crawlers",
        lambda: glue_crawlers.collect_crawlers(glue, window=window),
        _nothing(),
        count=len,
        data_through=_latest_data_through,
    ...
    account.glue_triggers = health.capture(
        "Glue Triggers",
        lambda: triggers.collect_triggers(glue),
        _nothing(),
        count=len,
    ...
    account.databrew_jobs = health.capture(
        "Glue DataBrew",
        lambda: databrew.collect_jobs(session.client("databrew"), window=window),
        _nothing(),
        count=len,
        data_through=_latest_data_through,
    ...
    account.interactive_sessions = health.capture(
        ...
        lambda: sessions.collect_sessions(glue, window=window),
        _nothing(),
        count=len,
        data_through=_latest_data_through,
    ...
    athena_analysis = health.capture(
        "Athena Queries",
        lambda: athena_monthly.collect_analysis(
    ...
    account.state_machines = health.capture(
        "Step Functions",
        lambda: stepfunctions.collect_state_machines(
    ...
    account.schedules = health.capture(
        ...
        lambda: schedules.collect_schedules(session.client("events")),
        _nothing(),
        count=len,
    ...
    if touches_table:
        stats = health.capture(
            "Table Touches",
            lambda: touches.collect_touches(
                session.client("athena"),
    ...
    if datawarm_job:
        health.capture(
            "DataWarm Mapping",
            lambda: _enrich_return(
                lambda: datawarm.mark_publications(account, datawarm_job),
                account.tables,
            ),
            _nothing(),
    ...
    if include_cloudtrail:
        account.actor_events = health.capture(
            ...
            lambda: cloudtrail.collect_actor_events(
